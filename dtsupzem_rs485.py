...
while True:
    ...
    ##Read DTSU666
    # Điện năng tiêu thụ 
    A_sum = round(instrument2.read_float(16424, functioncode=3)*1000, 2)
    A_imp = round(instrument2.read_float(0x401E, functioncode=3)*1000, 2)
    A_exp = round(instrument2.read_float(0x4028, functioncode=3)*1000, 2)
    #  Điện năng phản kháng Q
    Q1= round(instrument2.read_float(0x4032, functioncode=3)*1000, 2)
    Q2= round(instrument2.read_float(0x403C, functioncode=3)*1000, 2)
    Q3 = round(instrument2.read_float(0x4046, functioncode=3)*1000, 2)
    Q4 = round(instrument2.read_float(0x4050, functioncode=3)*1000, 2)
    # Điện áp dây
    ...
    Pft = round(instrument2.read_float(0x2012, functioncode=3)/10, 2)
    Pfa = round(instrument2.read_float(0x2014, functioncode=3)/10, 2)
    Pfb = round(instrument2.read_float(0x2016, functioncode=3)/10, 2)
    Pfc = round(instrument2.read_float(0x2018, functioncode=3)/10, 2)
    # Công suất phản kháng pha 
    Qft = round(instrument2.read_float(0x201A, functioncode=3)/10, 2)
    Qfa = round(instrument2.read_float(0x201C, functioncode=3)/10, 2)
    Qfb = round(instrument2.read_float(0x201E, functioncode=3)/10, 2)
    Qfc = round(instrument2.read_float(0x2020, functioncode=3)/10, 2)
    # Hệ số công suất pha 
    ...
    # Điện năng tiêu thụ 
    print('A_sum: {0:.2f} kWh'.format(A_sum/1000))
    print('A_imp: {0:.2f} kWh'.format(A_imp/1000))
    print('A_exp: {0:.2f} kWh'.format(A_exp/1000))
    # Điện năng phản kháng Q
    print('Q1: {0:.2f} kVArh'.format(Q1/1000))
    print('Q2: {0:.2f} kVArh'.format(Q2/1000))
    print('Q2: {0:.2f} kVArh'.format(Q3/1000))
    print('Q4: {0:.2f} kVArh'.format(Q4/1000))
    # Điện áp dây
    ...
    # Công suất tiêu thụ pha 
    print('Pft: {0:.2f} W'.format(Pft))
    print('Pfa: {0:.2f} W'.format(Pfa))
    print('Pfb: {0:.2f} W'.format(Pfb))
    print('Pfc: {0:.2f} W'.format(Pfc))
    # Công suất phản kháng pha
    print('Qft: {0:.2f} Var'.format(Qft))
    print('Qfa: {0:.2f} VAr'.format(Qfa))
    print('Qfb: {0:.2f} VAr'.format(Qfb))
    print('Qfc: {0:.2f} VAr'.format(Qfc))
    # Hệ số công suất pha 
    print('Cosft: {0:.2f} PF'.format(Cosft))
